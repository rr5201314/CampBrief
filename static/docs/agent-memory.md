...
- **前端派生视图（正文拆分）**：源文件 `static/data/daily-news.json` 是唯一事实源，**保持含 `detail`**（采集流程与 `validate-daily-news.py` 的 `REQUIRED_FIELDS` 都依赖它）。前端另读两个派生视图，均由 `scripts/build-daily-news-views.py` 生成：
  ...
  - `daily-news-home.json`：首页看板的精简 feed，只含 `priority>=3` 的条目（约 10KB gzip，源文件约 302KB）；筛选规则是「够用的超集」，近 3 天的时间窗仍由 `home.js` 按浏览器当前时间判断，脚本不重复实现
  - 详情页与技术板块继续读源文件，不受影响
...
- **同主题不等于重复**：候选与历史条目涉及同一模型、产品、公司或事件时，必须对比原文与已有 `title`/`summary`/`detail` 的关键事实，不能仅凭标题、关键词、实体或“上周已报道”直接丢弃。
- **同一消息**：只是重复转述，价格/额度/功能/状态/范围/生效时间没有变化 → 不新增。
- **叠加消息**：发生新的事实或新的生效动作（如再次降价、额度再次变化、正式上线、范围变化、状态推进、政策新增条款、融资金额更新）→ 保留旧条目，并作为独立新条目发布，写清前后数值、范围与时间。
- **价格消息必须分清作用域**：区分平台/供应商、官方 API、订阅套餐和 Credits，并核对旧价→新价、降幅、限时性和生效日期；晚发布不自动等于新变化，同一 URL 也不自动等于重复。
- **案例**：GPT-5.6 Sol 的 OpenRouter 半价与 OpenAI 官方 API/Codex Credits 后续降价是两次不同变更，不能按同话题合并。
