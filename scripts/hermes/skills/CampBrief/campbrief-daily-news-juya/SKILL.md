...
#### 3.0 历史消息与后续更新核验（必须先做）

juya 日报中的拆分候选即使与历史条目属于同一主题，也不能仅因“之前报道过”就丢弃。对同一模型、产品、公司、政策或事件，必须把日报原文与已发布条目的 `title`、`summary`、`detail` 做事实级对比；标题相似、关键词相同、同一个日报 URL 或同一个实体都不是自动去重依据。

- **同一消息**：只是重复转述或换来源，价格/额度/功能/状态/适用范围/生效时间等关键事实没有变化。保留更完整、核验更可靠的版本，不新增条目。
- **叠加消息 / 后续更新**：原文确认发生了新的事实或新的生效动作，例如再次降价、额度再次变化、套餐/适用范围变化、功能正式上线、阶段状态推进、政策新增条款、融资金额更新等。保留旧条目，并将新变化作为**独立新条目**发布；写清“再次/进一步/随后”、前后数值、适用范围和生效时间。juya 拆分条目即使共享日报 URL，只要事实是新的，仍应按独立标题和独立 `id` 收录。
- **澄清或纠错**：只是对同一次事件补充细节、纠正表述或换了来源，没有新的动作时，不新增条目；必要时更新已有条目的摘要/详情，保留同一个 `id`。
- **特别注意价格与权益**：必须区分平台/供应商与模型官方 API、订阅方案、Credits 等范围，也要比较旧价→新价、降幅、是否限时和生效日期。发布时间更晚本身不能证明是再次变更；同一个日报 URL 也不能证明是重复。
- 当前 30 天主文件没有命中时，按关键词定向检查 `static/data/daily-news-archive-*.json`，不要因历史条目已归档就跳过核验。
- 无法确认是否有实质新增时，先核验日报正文、候选中的独立链接和相关官方页面；仍不确定则标记待核验，不要武断归为重复。
...
本地候选池不提交到 Git。完成本地提交后，**必须先再次拉取远程更新，再推送**。只有 `static/data/daily-news.json` 已完成提交、最终 `git pull --ff-only` 与 `git push` 都成功后，才清空本次候选池；任何校验、提交、拉取或推送失败都必须保留候选池，供下次任务重试和排查。
...
- 本 skill 只处理 juya AI 日报源，其他源由 `campbrief-daily-news` skill 处理。两个 skill 共享同一个 `static/data/daily-news.json` 发布文件，但使用各自独立的本地候选池；成功发布后各自清空本次候选池。
