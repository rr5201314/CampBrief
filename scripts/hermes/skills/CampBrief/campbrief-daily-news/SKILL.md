...
候选与历史条目主题相同，**不等于**候选是重复消息。对涉及同一产品、模型、公司、政策或事件的候选，必须把候选原文与已发布条目的 `title`、`summary`、`detail` 做事实级对比；仅凭标题相似、关键词相同、实体相同或“上周已经报道过”不得直接丢弃。

- **同一消息**：新文章只是重复转述或补充来源，价格/额度/功能/状态/适用范围/生效时间等关键事实没有变化。保留信息更完整、原文更可靠的版本，不新增条目。
- **叠加消息 / 后续更新**：原文确认发生了新的事实或新的生效动作，例如再次降价、额度再次变化、套餐/适用范围扩大或缩小、功能正式上线、阶段状态推进、政策新增条款、融资金额更新等。此时保留旧条目，并把新变化作为**独立新条目**发布；标题、摘要和详情要写清“再次/进一步/随后”、前后数值、适用范围和生效时间。
- **澄清或纠错**：只是对同一次事件补充细节、纠正表述或换了来源，没有新的动作时，不新增条目；必要时更新已有条目的摘要/详情，保留同一个 `id`。
- **特别注意价格与权益**：必须区分平台/供应商（如 OpenRouter）与模型官方 API、订阅方案、Credits 等不同范围，也要比较旧价→新价、降幅、是否限时和生效日期。发布时间更晚本身不能证明是再次变更；反过来，同一模型或同一 URL 也不能证明是重复。
- 当前 30 天主文件没有命中时，按关键词定向检查 `static/data/daily-news-archive-*.json`，不要因为历史条目在归档文件就跳过核验。
- 无法确认是否有实质新增时，先打开候选原文和相关官方页面核实；仍不确定则保守标记为待核验，不要武断归为重复。
